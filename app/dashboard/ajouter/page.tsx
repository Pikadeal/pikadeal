...
export default function AjouterCarte() {
  ...
  async function rechercherCarte(query: string) {
    ...
    try {
      ...
      const res = await fetch(`https://api.pokemontcg.io/v2/cards?q=name:${nomTraduit}*&pageSize=8`)
    ...
    <main className="min-h-screen bg-white">
      ...
      <div className="px-8 py-8 max-w-lg">
        ...
        <div className="mb-6 relative">
          ...
          {recherching && (
            <p className="text-xs text-amber-500 mt-2">Recherche en cours...</p>
          )}
              ...
              {recherche.map((carte: any) => (
                ...
                >
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-300">{carte.name}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          {!recherching && nom.length >= 2 && recherche.length === 0 && !carteSelectionnee && (
            <p className="text-xs text-gray-400 mt-2">Aucune carte trouvée — essaie un autre nom</p>
          )}
        </div>
        ...
        {carteSelectionnee && (
          <div className="mb-6 flex items-center gap-4 p-4 bg-amber-50 rounded-xl border-2 border-amber-400">
            <img
              src={carteSelectionnee.images.small}
              alt={carteSelectionnee.name}
              className="w-16 object-contain rounded-lg flex-shrink-0"
            />
            <div>
